...
const PhoneInsert = ({ onClickSave }) => {
  ...
  input box에 클릭을 하고 키보드로 문자을 입력하면 한글자마다 (e)매개변수의
  e.target.value 값에 일단 저장이 된다.
  e.target.value 값에 저장된 문자열을 setName() 함수를 통하여 name state변수에
  저장을 해준다.
  */
  const nameChange = (e) => {
  ...
  화면이 Rendering될 때 수시로 onclickSave() 함수가 실행되어 React가 폭주하는
  현상이 발생을 해버린다
  ...
  */
  return (
    <form>
      ...
      <button
        onClick={({ name: { name } }, { number: { number } }) =>
          onClickSave(name, number)
        }
        type="button"
      >
        저장
